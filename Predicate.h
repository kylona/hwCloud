...
class Predicate {
protected:
  std::set<std::vector<std::string>> oldData;
...
public:
  ...
  std::set<std::vector<std::string>> findNewData(std::set<std::vector<std::string>> allData) {
    std::set<std::vector<std::string>> newData;
    for (auto item : allData) {
      if (oldData.insert(item).second) newData.insert(item);
    }
    return newData;
  }

  Token* getDataAt(int index) {
    return parameters.at(index)->getData();
  }
