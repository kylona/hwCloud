...
public:
  ...
  void toRelation(std::map<std::string, Relation*>& relations) {
    Relation* PastAll = NULL;
    Relation* PastNew = NULL;
    Relation* CurrentAll = NULL;
    Relation* CurrentNew = NULL;
    ...
    for (int i = 1; i < predicates.size(); i++) {
      ...
      try {
        ...
        this->activeR = relations.at(predicates[i]->getName())->copy();
        Relation* saveR = activeR->copy();
        CurrentAll = runQueryCommand(0);
        CurrentNew = CurrentAll->copyNewData(predicates[i]->findNewData(CurrentAll->getData()));

        if (PastAll != NULL) {
          if (PastAll->getSize() * CurrentAll->getSize() < (PastAll->getSize() * CurrentNew->getSize() + PastNew->getSize() * CurrentAll->getSize()) ) {
            PastAll->naturalJoin(CurrentAll);
          }
          else {
            PastAll->naturalJoin(CurrentNew);
            PastNew->naturalJoin(CurrentAll);
            PastAll->merge(PastNew);
          }
